...
function SearchItem({name='BTC', setOpen, isFavorite, onToggleFavorite}) {
  return (
    <li className='flex font-thin w-full px-4 py-1.5 hover:bg-textGray cursor-pointer'>
        <button onClick={onToggleFavorite}>
            ...
            className='h-[24px] mr-[8px]' 
            src={isFavorite ? starFull : starEmpty} 
            alt={isFavorite ? 'Unfavorite' : 'Favorite'}
            />
        </button>
        <span className='w-full pt-1' onClick={setOpen}>{name}</span>
    </li>
  )
}
